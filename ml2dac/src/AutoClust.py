...
related_work_path = Path("Experiments/RelatedWork/related_work")
related_work_result_path = Path("gen_results/evaluation_results/synthetic_data/related_work")
...
def rw_run_offline_phase():
    # Build a config space with the hyperparameters for each algorithm separately
    cs_per_algo = ClusteringCS.build_paramter_space_per_algorithm()
    # Clustering algorithms to evaluate
    clustering_algos = ['ward', 'dbscan', 'birch', 'spectral', 'means_shift', 'affinity_propagation']
    related_work_csv_path = related_work_path / 'autoclust_work_offline_opt.csv'

    # Check if the file already exists
    file_exists = os.path.exists(related_work_csv_path)
    ...
    for algo in clustering_algos:
        for X, y, dataset_name in zip(datasets, true_labels, d_names):
            ...
            if not file_exists:
                rw_opt_result_df.to_csv(related_work_csv_path, mode='w', index=False)
                file_exists = True  # Ensure subsequent writes only append
            else:
                rw_opt_result_df.to_csv(related_work_csv_path, mode='a', header=False, index=False)

            print(rw_opt_result_df.columns)
            print(rw_opt_result_df)
